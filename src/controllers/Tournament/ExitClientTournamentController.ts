...
class ExitClientTournamentController {
  async handle(req: Request, res: Response) {
    ...
    if (award) {
      let valueDebit =
        methods_transaction.filter((item) => item["id"] == "Pag Dívida")
          .length != 0
          ? methods_transaction.filter((item) => item["id"] == "Pag Dívida")[0]
              .value
          : 0;
      let valueCredit =
        methods_transaction.filter((item) => item["id"] == "Crédito").length !=
        0
          ? methods_transaction.filter((item) => item["id"] == "Crédito")[0]
              .value
          : 0;
