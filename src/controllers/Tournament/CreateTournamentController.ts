...
class CreateTournamentController {
    async handle(req: Request, res: Response) {
        const { name, timechip, chairs, totalAward_guaranteed, timer_round, timer_interval, show_max, is_rebuy, 
            rounds_to_interval, max_rebuy, rake, nivel_max_in, nivel_max_timechip, percentage_players_award,
            purchases
        } = req.body
        ...
        const tournament = await createTournamentService.execute({
            name, timechip, chairs, totalAward_guaranteed, intervals, club_id, rake, max_rebuy,
            nivel_max_timechip, percentage_players_award, is_rebuy, show_max, nivel_max_in,
            purchases
        })
