...
class CreateTournamentService {
    async execute({ name, timechip, chairs, totalAward_guaranteed, intervals, max_rebuy, rake, nivel_max_in,
        nivel_max_timechip, percentage_players_award, show_max, is_rebuy, club_id, purchases
    }: TournamentRequest) {
        
        if (!name || !timechip || !chairs || !totalAward_guaranteed || !intervals || !rake || !nivel_max_in || !percentage_players_award || !club_id){
            throw new Error("Preencha os campos obrigatórios")
        }

        if(!purchases.some(data => data["type"] == "service") || !purchases.some(data => data["type"] == "purchase") || !purchases.some(data => data["type"] == "entrie")){
            throw new Error("Preencha pelo menos uma opção de compra em Entrada, Compras e Serviços")
        } 
