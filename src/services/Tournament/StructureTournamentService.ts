...
class StructureTournamentService {
    async execute({ club_id, intervals, nivel_max_buyin_discount, nivel_max_in, nivel_max_timechip,  blinds, tournament_id }: TournamentRequest) {
        ...
        if (!nivel_max_buyin_discount) {
            nivel_max_buyin_discount = 0
        }
        ...
        intervals.split("-").map((item, index) => {
            if (index < nivel_max_buyin_discount) {
        ...
        const tournamentEdit = await prismaClient.tournament.update({
            ...
            data: {
                ...
                datetime_max_buyin_discount: addMinutes(new Date(tournament.datetime_initial), minsBuyinDiscount),
