...
class ListTransactionsService {
  async execute({ club_id, page, filter }: TransactionRequest) {
    ...
    club["transactionsTotalReceive"] = transactionsTotalReceive.length
      ? transactionsTotalReceive
          .map((prod) => prod.value - prod.value_paid)
          .reduce((total, preco) => total + preco)
      : 0;
    club["transactionsTotalDebt"] = transactionsTotalDebt.length
      ? transactionsTotalDebt
          .map((prod) => prod.value - prod.value_paid)
          .reduce((total, preco) => total + preco)
      : 0;
