...
exports.PaymentPendingService = void 0;
...
class PaymentPendingService {
  async execute({
    ...
    methods_transaction = methods_transaction.filter(item => item["id"] != "Crédito");
    await Promise.all(client.transactions.map(async item => {
      let {
        payCredit,
        methodsPay,
        methodsC
      } = await (0, _functions.getMethodsPay)(item.value - item.value_paid, methods_transaction);
    ...
    if (totalPaidClub || totalPaidDealer || totalPaidPassport || totalPaidJackpot) {
      await _prisma.default.club.update({
        ...
        data: {
          ...
          passport: parseFloat((client.club.passport + totalPaidPassport).toFixed(2)),
          jackpot: parseFloat((client.club.jackpot + totalPaidJackpot).toFixed(2))
        }
      });
    }
...
exports.PaymentPendingService = PaymentPendingService;